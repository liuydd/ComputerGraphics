...
void SceneParser::parseFile() {
    ...
    while (getToken(token)) {
        if (!strcmp(token, "PerspectiveCamera")) {
            ...
            group = parseGroup();
        } else if (!strcmp(token, "DofCamera")) {
            parseDofCamera();
        } else {
...
void SceneParser::parseDofCamera() {
    ...
    getToken(token);
    assert (!strcmp(token, "}"));
    camera = new DofCamera(center, direction, up, width, height, angle_radians, distance, r);
}
...
Material *SceneParser::parseMaterial() {
    char token[MAX_PARSER_TOKEN_LENGTH];
    char texturefilename[MAX_PARSER_TOKEN_LENGTH] = {0};
    char bumpfilename[MAX_PARSER_TOKEN_LENGTH] = {0};
    Vector3f diffuseColor(1, 1, 1), specularColor(0, 0, 0), emission(0, 0, 0), color(1, 1, 1);
    Vector4f type(1, 0, 0, 0);
    float shininess = 0;
    float refr = 1;
    getToken(token);
    assert (!strcmp(token, "{"));
    ...
    auto *answer = new Material(diffuseColor, specularColor, shininess, color, emission, refr, type, texturefilename, bumpfilename);
    return answer;
}
